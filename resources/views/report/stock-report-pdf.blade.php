    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 9%;">Stock Value</th>
            </tr>
        </thead>
        <tbody>
            @php
            ...
            @foreach ($stocks as $key => $item)
                @php
                    ...
                    $currentStock = $opening + $purchase + $production + $sreturn 
                ...
                <tr>
                    ...
                    <td class="text-end">{{ $opening > 0 ? number_format($opening, 2) . ' ' . $item->stock_unit : '-' }}</td>
                    <td class="text-end">{{ $purchase > 0 ? number_format($purchase, 2) . ' ' . $item->stock_unit : '-' }}</td>
                    <td class="text-end">{{ $production > 0 ? number_format($production, 2) . ' ' . $item->stock_unit : '-' }}</td>
                    <td class="text-end">{{ $sales > 0 ? number_format($sales, 2) . ' ' . $item->stock_unit : '-' }}</td>
                    <td class="text-end">{{ $consume > 0 ? number_format($consume, 2) . ' ' . $item->stock_unit : '-' }}</td>
                    <td class="text-end">{{ $preturn > 0 ? number_format($preturn, 2) . ' ' . $item->stock_unit : '-' }}</td>
                    <td class="text-end">{{ $sreturn > 0 ? number_format($sreturn, 2) . ' ' . $item->stock_unit : '-' }}</td>
                    <td class="text-end">{{ $damage > 0 ? number_format($damage, 2) . ' ' . $item->stock_unit : '-' }}</td>
                    <td class="text-end" style="font-weight: bold;">{{ number_format($currentStock, 2) }} {{ $item->stock_unit }}</td>
            ...
            @if(count($stocks) == 0)
                <tr>
                    <td colspan="14" style="text-align: center; padding: 20px;">No data found</td>
                </tr>
            @endif
        </tbody>
        <tfoot>
            <tr class="total-row">
                <td colspan="13" style="text-align: right; font-weight: bold;">Total Stock Value:</td>
                <td class="text-end" style="font-weight: bold;">{{ number_format($totalValue, 2) }}</td>
            </tr>
        </tfoot>
    </table>
