    ...
    <div class="app-content">
        <div class="container-fluid">
            <div class="card card-primary card-outline">
                <!--begin::Form-->
                <form id="salesForm" action="{{ route('sales.update', $sales->id) }}" method="POST">
                    @csrf
                    <!--begin::Body-->
                    <div class="card-body">
                        <!-- Hidden URLs -->
                        ...
                        <div id="customerContainer" class="border-0 shadow-sm ms-0">
                            ...
                            <div class="row g-2 align-items-end mb-1">
                                <div class="form-group col-md-5 mb-1 position-relative">
                                    <input type="text" name="c_name" value="{{ $sales->customer->c_name ?? '' }}"
                                        class="form-control c_name" id="c_name" placeholder=" " required>
                                    <label for="c_name" class="floating-label">Customer Name</label>
                                    <input type="hidden" name="customer_id" class="customer_id"
                                        value="{{ $sales->customer->id ?? '' }}" id="customer_id">
                                </div>
                                <div class="form-group col-md-3 mb-1">
                                    <input type="text" name="phone" class="form-control phone"
                                        value="{{ $sales->customer->phone ?? '' }}" id="phone" placeholder=" "
                                        required>
                                    <label for="phone" class="floating-label">Phone</label>
                                </div>
                                <div class="form-group col-md-4 mb-1">
                                    <input type="email" name="email" class="form-control email"
                                        value="{{ $sales->customer->email ?? '' }}" id="email" placeholder=" ">
                                    <label for="email" class="floating-label">Email</label>
                                </div>
                                <div class="form-group col-md-8 mb-1">
                                    <textarea name="address" class="form-control address" id="address" rows="1" placeholder=" " required>{{ $sales->customer->address ?? '' }}</textarea>
                                    <label for="address" class="floating-label">Address</label>
                                </div>
                                <div class="form-group col-md-2 mb-1">
                                    <input type="date" name="date" class="form-control date" id="date"
                                        value="{{ optional($sales->date)->format('Y-m-d') }}" required>
                                ...
                                <div class="form-group col-md-2 mb-1">
                                    <input type="text" name="invoice_no" class="form-control invoice_no" id="invoice_no"
                                        value="{{ $sales->invoice_no }}" placeholder=" " required>
                                ...
                                <div class="sales-item-row row g-2 align-items-end mb-2">
                                    ...
                                    <div class="form-group col-md-1 mb-1">
                                        <input type="number" step="0.01" name="item_vat_amt[]"
                                            value="{{ $salesItem->item_vat_amt ?? 0 }}" class="form-control item_vat_amt"
                                            placeholder=" ">
                                    ...
                                    <div class="form-group col-md-2 d-flex align-items-end mb-1">
                                        <div class="w-100">
                                            <input type="number" step="0.01" name="total_price[]"
                                                value="{{ $salesItem->total_price }}" class="form-control total_price"
                        ...
                        <div class="row g-2">
                            <div class="form-group col-md-1 mt-2 mb-2">
                                <input type="number" step="0.001" name="dis_percent"
                                    value="{{ $sales->dis_percent ?? 0 }}" class="form-control dis_percent"
                                    id="dis_percent" placeholder=" ">
                                <label class="floating-label">Discount(%)</label>
                            </div>
                            <div class="form-group col-md-1 mt-2 mb-2">
                                <input type="number" step="0.01" name="dis_amt" value="{{ $sales->dis_amt ?? 0 }}"
                            ...
                            <div class="form-group col-md-2 mt-2 mb-2">
                                <input type="number" step="0.01" name="sub_total"
                                    value="{{ $sales->sub_total ?? 0 }}" class="form-control sub_total"id="sub_total"
                                    placeholder=" " readonly>
                                <label class="floating-label">Sub Total</label>
                            </div>
                            <div class="form-group col-md-2 mt-2 mb-2">
                                <input type="number" step="0.01" name="vat_amt" value="{{ $sales->vat_amt ?? 0 }}"
                                    class="form-control vat_amt" id="vat_amt" placeholder=" " readonly>
                                <label class="floating-label">VAT Amount</label>
                            </div>
                            <div class="form-group col-md-2 mt-2 mb-2">
                                <input type="number" step="0.01" name="grand_total"
                                    value="{{ $sales->grand_total ?? 0 }}" class="form-control grand_total"
                                    id="grand_total" placeholder=" " readonly>
                                <label class="floating-label">Grand Total</label>
                            </div>
                            <div class="form-group col-md-2 mt-2 mb-2">
                                <input type="number" step="0.01" name="paid_amt" value="{{ $totalPaid ?? 0 }}"
                                    class="form-control paid_amt" id="paid_amt" placeholder=" ">
                                <label class="floating-label">Received Amount</label>
                            </div>
                            <div class="form-group col-md-2 mt-2 mb-2">
                                <input type="number" step="0.01" name="due_amt" value="{{ $due ?? 0 }}"
                                    class="form-control due_amt" id="due_amt" placeholder=" " readonly>
                                <label class="floating-label">Due</label>
                            </div>
                            <div class="form-group col-md-2 mb-2">
                                <input type="text" name="reference" value="{{ $sales->reference ?? '' }}"
                                    class="form-control reference" placeholder=" ">
                                <label class="floating-label">Reference</label>
                            </div>
                            <div class="form-group col-md-2 mb-2">
                                <input type="text" name="pay_receive" value="{{ $sales->pay_receive ?? '' }}"
                                    class="form-control pay_receive" placeholder=" ">
                            ...
                            <div class="form-group col-md-2 mb-2">
                                <select name="payment_method" class="form-control payment_method" required>
                                    ...
                                    <option value="cash" {{ $sales->payment_method == 'cash' ? 'selected' : '' }}>
                                        Cash
                                    </option>
                                    <option value="bank" {{ $sales->payment_method == 'bank' ? 'selected' : '' }}>
                                        Bank
                                    </option>
                                    <option value="cheque" {{ $sales->payment_method == 'cheque' ? 'selected' : '' }}>
                                        Cheque
                                    </option>
                                    <option value="mobile_bank"
                                        {{ $sales->payment_method == 'mobile_bank' ? 'selected' : '' }}>
                                        Mobile Banking
                                    </option>
                                    <option value="due" {{ $sales->payment_method == 'due' ? 'selected' : '' }}>
                                        Due
                                    </option>
                                </select>
                            ...
                            <div class="form-group col-md-2 mb-2">
                                <select name="payment_status" class="form-control payment_status" required>
                                    ...
                                    <option value="paid" {{ $sales->payment_status == 'paid' ? 'selected' : '' }}>
                                        Paid
                                    </option>
                                    <option value="unpaid" {{ $sales->payment_status == 'unpaid' ? 'selected' : '' }}>
                                        Unpaid
                                    </option>
                                    <option value="partial" {{ $sales->payment_status == 'partial' ? 'selected' : '' }}>
                                        Partial
                                    </option>
                                </select>
                                ...
                                <select name="payment_account_id" class="form-control payment_account_id">
                                    ...
                                    @foreach ($accounts as $account)
                                        <option value="{{ $account->id }}"
                                            {{ $sales->payment_account_id == $account->id ? 'selected' : '' }}>
                        ...
                        <div class="card-footer d-flex align-items-end justify-content-between gap-3 p-0 m-0">
                            <div class="form-group flex-grow-1 mt-2">
                                <textarea name="narration" class="form-control narration" rows="1" placeholder=" " required>{{ $sales->narration ?? '' }}</textarea>
...
@push('script')
    <script>
        ...
        $(document).ready(function() {
            ...
            // Calculate existing sales values on edit page
            container.find('.sales-item-row').each(function() {
                recalcRow($(this));
            });
            ...
            console.log('✅ Sales Edit Calculator initialized successfully!');
        });
    </script>
    @if (session('error'))
        <script>
